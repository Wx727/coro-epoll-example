...
void IoContext::UpdateState(Socket* socket, uint32_t new_state) {
    if (socket->io_state != new_state) {
        struct epoll_event ev;
        ev.events = new_state;
        ev.data.ptr = socket;
        ...
        socket->io_state = new_state;
    }
}

void IoContext::WatchRead(Socket* socket) {
    auto new_state = socket->io_state | EPOLLIN;
...
void IoContext::WatchWrite(Socket* socket) {
    auto new_state = socket->io_state | EPOLLOUT;
    UpdateState(socket, new_state);
}
